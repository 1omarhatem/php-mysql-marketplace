...
if (isset($_POST['submit'])) {
    ...
    if ($existingUser->num_rows > 0) {
        $msg = [
            "title" => "Error!",
            "text" => "Existing username, email, or phone number."
        ];
    } else {
...
<body>
    ...
    <script>
        $(document).ready(function() {
            $('input[name="number"]').on('input', function() {
                ...
                if (number.length > 0 && !/^\d+$/.test(number)) {
                    $('#number-error').text('Number must consist of digits only.');
                } else {
                    $('#number-error').text('');
                }
            });
        ...
        $(document).ready(function() {
            function isUsernameExists(value, callback) {
                $.ajax({
            ...
            $('input[name="username"]').on('input', function() {
                ...
                if (username.length > 0) {
                    isUsernameExists(username, function(result) {
                        if (result) {
